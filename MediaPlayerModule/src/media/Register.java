...
public class Register {
    ...
    private VBox RegisterVBox;// 父窗体中显示歌单列表的VBox对象
    ...
    public Register(Stage parentStage, MusicPlayerMainApp musicPlayerMainApp) {
        this.parentStage = parentStage;
        this.RegisterVBox = RegisterVBox;
        this.musicPlayerMainApp = musicPlayerMainApp;

        // 1.注册：Label
        Label labRegister = new Label("注册");
        labRegister.setTextFill(Color.WHITE);
        labRegister.setFont(new Font("黑体", 18));
        labRegister.setPrefWidth(160);
        labRegister.setPrefHeight(100);
        labRegister.setLayoutX(120);
        labRegister.setLayoutY(-30);
        // 绽放效果
        Bloom bloom = new Bloom();
        bloom.setThreshold(0.5);
        labRegister.setEffect(bloom);
        ...
        // 3.文本框：TextField----用户名&密码&邮箱&电话号码
        ...
        TextPassword.setLayoutY(105);

        TextField TextMail = new TextField();
        TextMail.setPromptText("请输入邮箱");
        TextMail.setPrefWidth(220);
        TextMail.setPrefHeight(15);
        TextMail.setLayoutX(20);
        TextMail.setLayoutY(140);

        TextField TextPhone = new TextField();
        TextPhone.setPromptText("请输入手机号");
        TextPhone.setPrefWidth(220);
        TextPhone.setPrefHeight(15);
        TextPhone.setLayoutX(20);
        TextPhone.setLayoutY(175);

        // 4.提示标签：Label----用户名&密码&邮箱&电话号码
        ...
        butOk.setOnMouseClicked(e -> {
            ...
            String txtMail = TextMail.getText().trim();
            // 判断
            if (txtMail == null || txtMail.length() == 0) {
                labMsg.setText("请输入邮箱！");
                return;
            }

            String txtPhone = TextPhone.getText().trim();
            // 判断
            if (txtPhone == null || txtPhone.length() == 0) {
                labMsg.setText("请输入手机号！");
                return;
            }

            String[] collectInfo=new String[4];
            collectInfo[0]=txtUser;
            collectInfo[1]=txtPassword;
            collectInfo[2]=txtMail;
            collectInfo[3]=txtPhone;
            EventHandling.giveOutMsg("REGIST",collectInfo);
            // 写入MusicGroup.xml中
...
//			this.RegisterVBox.getChildren().add(hBox1);
        ...
        login.getChildren().addAll(labRegister, labClose, txtUserName, TextPassword, TextMail, TextPhone, labMsg, butOk,
                butCancel);
        Scene sceneLogin = new Scene(login, 300, 250);
